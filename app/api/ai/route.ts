  ...
  try {
    ...
    if (!action || !content) {
      return NextResponse.json(
        { error: "Missing required fields", message: "Please provide action and content" },
        { status: 400 },
      )
    }

    if (!["summarize", "explain", "questions"].includes(action)) {
      return NextResponse.json(
        { error: "Invalid action", message: "Action must be summarize, explain, or questions" },
        { status: 400 },
      )
    }
    ...
    if (typeof content !== "string" || content.trim().length === 0) {
      return NextResponse.json(
        { error: "Invalid content", message: "Content must be a non-empty string" },
        { status: 400 },
      )
    }
    ...
    const result = await generateAIResponse(action, content)
